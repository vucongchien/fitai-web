...
(function(){try{if(window.opener&&!window.opener.closed){window.opener.postMessage(${JSON.stringify(message)},${JSON.stringify(origin)});window.close();return;}}catch(e){}window.location.assign(${JSON.stringify(safeFallback)});})();
